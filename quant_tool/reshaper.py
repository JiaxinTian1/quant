...
class ReshaperFactory():
    ...
    def create(granularity) -> "BaseScaler":
        if granularity == 'tensor':
            return TensorReshaper()
        elif granularity == 'output_channel':
            return OutputChannelReshaper()
        elif granularity == 'input_channel':
            return InputChannelReshaper()
...
class TensorReshaper(BaseReshaper):

    def reshape(self, tensor: torch.Tensor):
        self.original_shape = tensor.shape
        return tensor.unsqueeze(0).reshape(1, -1)

    def unreshape(self, tensor: torch.Tensor):
        return tensor.reshape(self.original_shape)


class OutputChannelReshaper(BaseReshaper):
...
class InputChannelReshaper(BaseReshaper):
    ...
    def reshape(self, tensor: torch.Tensor):
        ...
        # 将最后一个维度移到最前面
        transposed = tensor.permute(ndim - 1, *range(ndim - 1))
        
        # 保存信息用于unreshape
        self.transposed_shape = transposed.shape
        self.permute_order = (ndim - 1,) + tuple(range(ndim - 1))
        self.inverse_permute_order = tuple(range(1, ndim)) + (0,)
    ...
    def unreshape(self, tensor: torch.Tensor):
        # 先恢复转置后的形状
        transposed = tensor.reshape(self.transposed_shape)
        
        # 再反向转置恢复原始顺序
        original = transposed.permute(*self.inverse_permute_order)
        
        return original.reshape(self.original_shape)
